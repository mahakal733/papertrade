...
const data=useSelector((state) => {
  ...
  return (
    <div className='trade2'>
     ...
     {
      data.map((element) => element?<Entity key={element.name+element.exchange} name={element.name} current_price={element.current_price} daychange={element.daychange} exchange={element.exchange} exists={true} watchlist={1}/>:""
      )
     }
     </div>
    </div>
