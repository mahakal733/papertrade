...
const entity = ({ name, daychange, current_price, exchange, exists, watchlist }) => {
  ...
  const change_p = daychange * 100 / (current_price - daychange)
  ...
  return (
    <div className='entity0'>
      ...
      <div className='order0'>
        ...
        {
          exists ? <button
            onClick={(params) => dispatch(RemoveFromWatchlist({ name, exchange, watchlistNumber: watchlist }))}
            className='remove' >remove</button> :
            <button
              onClick={(params) => dispatch(AddToWatchlist({ name, exchange, watchlistNumber: watchlist, current_price, daychange }))}
              className='remove' >add</button>
        }
      </div>
      <div className='entity1'>
        <div className='currentprice' >{current_price}</div>
        <div className={change_p > 0 && 'green' || change_p < 0 && 'red' || change_p === 0 && 'gray'}>{daychange}  ({parseFloat(change_p.toFixed(2))}%)</div>
